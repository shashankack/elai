...
import { optInPlatformShippingWorkflow } from "../../../../workflows/platform-shipping"
import { VendorOptInPlatformShippingType } from "../validators"

export const POST = async (
  req: AuthenticatedMedusaRequest<VendorOptInPlatformShippingType>,
  ...
  const { result } = await optInPlatformShippingWorkflow(req.scope).run({
    input: {
      seller_id: sellerId,
      stock_location_id,
  ...
  res.status(201).json({ platform_shipping_opt_in })
}
